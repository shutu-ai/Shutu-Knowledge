...
The repository/package `config.yaml` is a secret-free Knowledge baseline and
template. The effective user-editable file is `<data home>/config.yaml`; it is
generated by `shutu-knowledge doctor --init` and remains entirely independent
of `shutu-agent/config.yaml`. The data home defaults to `~/.shutu/knowledge`;
set `SHUTU_KNOWLEDGE_HOME` to override it. Unknown YAML fields fail startup
