...
func TestParseTemporalQueryCategories(t *testing.T) {
	cases := []struct{ query string; expected TemporalIntent; versions []string }{
	...
	for _, test := range cases {
		...
		for i, version := range test.versions {
			if parsed.Versions[i] != version { t.Fatalf("version %d=%q want %q",i,parsed.Versions[i],version) }
		}
	}
}
...
func TestTemporalCompilationSupersedesAndKeepsProvenance(t *testing.T) {
	...
	for i := range compilation.Units {
		...
		if unit.Type != UnitFact { continue }
		switch {
		case strings.Contains(unit.Content,"128k"): oldUnit=unit
		case strings.Contains(unit.Content,"1M"): newUnit=unit
		}
	}
	if oldUnit==nil || newUnit==nil { t.Fatalf("versioned facts missing") }
	...
	for _, relation := range compilation.Relations {
		if relation.Type == RelationSupersedes && relation.SubjectUnitID==oldUnit.ID && relation.ObjectUnitID==newUnit.ID {
			supersedes++
			if len(relation.Sources)==0 { t.Fatal("supersession relation lacks provenance") }
		}
	}
	if supersedes != 1 { t.Fatalf("supersedes relations=%d want 1",supersedes) }
}
...
func TestTemporalSupersessionAuditExhaustivelyChecksThirtyPairs(t *testing.T) {
	for i := 0; i < 30; i++ {
		...
		for _, relation := range compilation.Relations {
			if relation.Type == RelationSupersedes { relations = append(relations,relation) }
		}
		if len(relations)!=1 {
...
func findTemporalAuditUnit(units []Unit,id string) Unit {
	for _, unit := range units { if unit.ID==id { return unit } }
	return Unit{}
}
