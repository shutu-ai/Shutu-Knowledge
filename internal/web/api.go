...
func registerKnowledgeAPI(mux *http.ServeMux, s *Server) {
	...
	mux.HandleFunc("POST /api/bases/{id}/schema/compile", s.compileBaseSchema)
	mux.HandleFunc("POST /api/bases/{id}/schema/search", s.searchSchema)
	mux.HandleFunc("POST /api/bases/{id}/schema/requirements", s.resolveDataRequirement)
	mux.HandleFunc("GET /api/bases/{id}/schema/tables/{tableId}", s.getSchemaTable)
	mux.HandleFunc("GET /api/bases/{id}/schema/fields/{fieldId}", s.getSchemaField)
